...
export class AdminFormComponent implements OnInit, OnChanges {
  ...
  validateForm(form) {
    ...
    if (this.mode == "add") {
      ...
      if (prod_name.length < 3 || form.controls.prod_name.pristine) {
        this.warning = "Product name must be at least 3 characters";
        return false;
      }
    ...
    if (this.mode == "edit") {
      ...
      if (!form.controls.prod_name.pristine && (prod_name && prod_name.trim()) && prod_name.length < 3) {
        this.warning = "Product name must be at least 3 characters";
        return false;
      }
